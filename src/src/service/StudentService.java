...
public class StudentService {
    ...
    // 수강 신청 내역 가져와 출력
    public boolean showLectureRegistrationList() throws IOException {
        //수강 신청 내역 가져오기
        //필터링 (해당 학생의 수강 신청 내역)
    ...
    // 수강 신청
    public void registerLectureRegistration(String choiceLectureId) throws IOException {
    ...
    // 수강 취소
    public void deleteLectureRegistration(String lectureId) throws IOException {
        ...
        for (int i = 0; i < studyRoom.getCheckSeat().length; i++) {
            for (int j = 0; j < studyRoom.getCheckSeat()[i].length; j++) {
                ...
                if (studyRoom.getCheckSeat()[i][j]) {
                    ...
                    if (reservationInformation != null && reservationInformation.equals((i + 1) + "-" + (j + 1)))
                        System.out.printf("[%s] ", "내좌석");
    ...
    public boolean reserveStudyRoom(String choiceSeat) {
